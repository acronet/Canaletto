...
#include "devices/DS18B20/DS18B20_2.h"
...
typedef struct DL_LOG_ITEM {
	...
	PM10QBIT_STATS PM10stats;// = {.mean = 0.0f, .samples = 0, .maxVal = 0, .minVal = 0};
	ALPHASENSE_STATS O3stats;// = {.aux = 0 , .working = 0};
	int16_t tempO3;
	int16_t tempShield;
	uint16_t vBat;
	uint8_t flags;	
...
uint16_t send_data_with_post( DL_SEND_PARAMS * const pPara )
{
	...
	//////////////////////////////
		...
		snprintf_P(	szBuf+post_data_end ,
					...
					PSTR("#TIME=%lu&B=%u&T_O3=%d&O3w=%u&O3a=%u&T=%u&PM10_s=%u&PM10=%d") ,
					ds.data_timestamp ,
					ds.vBat ,
					ds.tempO3 ,
					ds.O3stats.working ,
					ds.O3stats.aux ,
					ds.tempShield ,
					ds.PM10stats.samples ,
					ds.PM10stats.mean );
					
		post_data_end += strnlen(szBuf+post_data_end,256);

		for (uint8_t i = 0;i<sizeof(ds.PM10stats.valDistr)/sizeof(ds.PM10stats.valDistr[0]);i++){
			snprintf_P(szBuf+post_data_end,BUFFER_SIZE-post_data_end,PSTR("&V%u=%u"),i,ds.PM10stats.valDistr[i]);
			post_data_end += strnlen(szBuf+post_data_end,256);
		}
